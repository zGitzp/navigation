//来源：https://www.52pojie.cn/thread-2057144-1-1.html
//需求：需要autoxjs v7版本才行，新增了ocr文字识别是否需要点击跳转广告

//参考文献1:https://www.52pojie.cn/thread-1820936-1-1.html
//参考文献2：https://www.52pojie.cn/thread-1869154-1-1.html
...
if (auto.service == null) {
  ...
  if (
    ...
    log("起点已启动成功");
    sleep(2000);
    var thread = threads.start(function time() {
      //计时
      let shuzi = 0;
      do {
        sleep(1000);
    ...
    if (text("福利中心").exists()) {
      ...
      do {
        ...
        !(
          className("android.view.View").text("每日视频福利").exists() ||
          text("看视频得奖励").exists()
        )
      );
      log("已进入福利中心");
      ...
      if (textContains("看视频").exists()) {
        do {
          ...
          if (text("看视频").exists()) {
            ...
            //className("android.widget.Button").textContains("看视频").click();
            video_look();
            sleep(2000);
...
function video_look() {
  ...
  let closeBtn = null;
  let x3 = device.width * 0.85,
    y3 = device.height * 0.16;
  ...
  do {
    ...
    if (textContains("可获得奖励").exists()) {
      ...
      log("———————当前为静态广告———————");
      console.log("isNewLayout", isNewLayout);
      break;
    } else {
      isNewLayout = classNameContains("ViewGroup").find().length > 30;
      log("缓冲……");
    }
    if (
      m > 5 &&
      !(
        textContains("领取奖励").exists() || textContains("可获得奖励").exists()
      )
    ) {
      return;
    }
  } while (!textContains("可获得奖励").exists() && !isNewLayout);
  //获取退出坐标
  var video_quit = "";
  var x1 = 1;
  var x2 = 1;
  var y1 = 1;
  var y2 = 1;
  console.log("计算退出坐标开始");
  var thread = threads.start(function coordinate() {
    sleep(3000);
    if (textContains("可获得奖励").exists() && !video_quit) {
      ...
      console.log("计算退出坐标失败，稍后重新获取(静态广告不影响)");
    }
  });
  ...
  do {
    ...
    if (n == 1) {
      if (isNewLayout) {
        ...
        sleep(1000)
      } else click(parseInt((x1 + x2) / 2), parseInt((y1 + y2) / 2));
    ...
    !(
      textContains("福利中心").exists() ||
      textContains("视频奖励").exists() ||
      text("签到").exists()
    )
  );
  log("关闭视频");
